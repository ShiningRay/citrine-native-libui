...
| `on_enter` | ⚠️ | 核心已接（v0.3）：GTK 后端绑 entry 的 `activate` ✅；**libui 的 entry 不暴露按键事件** → 接线处 warn-once 后忽略（跨后端应用不炸），回车提交在本后端仍不可用，demo 改用按钮提交（`market` 的下单、`sheets` 的公式栏） |
...
| `on_wheel` | ⚠️ | 核心已接（v0.3，载荷 `{delta_x:, delta_y:, modifiers:}`）：GTK 后端走 `scroll-event` 归一并过滤滚轮键 4-7 ✅；**libui-ng 的 area 不投递滚轮**（`uiAreaMouseEvent` 无滚轮字段）→ 接线处 warn-once 后忽略；滚动请用 `scroll: true` + `AreaHandle#scroll_to`（`native-area.md` §5.4） |
...
2. **`on_wheel` 在核心有、本后端永不触发**（libui 的能力边界，不是实现进度）：接线处 warn-once 后忽略，不炸跨后端应用。
...
| `on_enter`（entry 回车） | 核心已接；本后端 warn-once 忽略 | uiEntry 不暴露按键的事实不变；候选自绘输入框（N4） |
